...
class SlimeVolleyEnv(gym.Env):
  ...
  def getObs(self):
    if self.from_pixels:
      ...
      obs_agent = self.game.agent_right.getObservation()
      obs_opponent = self.game.agent_left.getObservation()
      obs = np.stack([obs_agent, obs_opponent], axis=0)
    return obs
  ...
  def step(self, all_actions):
    ...
    return obs, [reward, -reward], [done, done], [info_agent, info_opponent]
