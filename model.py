...
class ActorCriticModel(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train, nsteps, ent_coef, vf_coef,
        ...
        vf_loss = .5 * tf.reduce_mean(IS_weight * vf_losses)

        # Final PG loss
        pg_losses = ADV * neglogpac
        pg_loss = tf.reduce_mean(IS_weight * pg_losses)
    ...
    def train(self, lr, obs, returns, masks, actions, values, neglogpacs, rewards, IS_weight, states=None):
        # Here we calculate advantage A(s,a) = R + yV(s') - V(s)
        # Returns = R + yV(s')
        advs = returns - values
